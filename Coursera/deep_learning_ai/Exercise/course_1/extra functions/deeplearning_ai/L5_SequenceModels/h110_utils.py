...
def rnn_cell_backward(da_next, cache):
    ...
    a = 1- np.square(np.tanh(a_next))
    print("a.shape:", a.shape)
    print("da_next.shape:", da_next.shape)
...
def lstm_cell_backward(da_next, dc_next, cache):
    ...
    #     dit = np.dot(dit,np.concatenate((a_prev,xt),axis=0).T)
    #     dft = np.dot(dft,np.concatenate((a_prev,xt),axis=0).T)
    #     dot = np.dot(dot,np.concatenate((a_prev,xt),axis=0).T)
    #     dcct = np.dot(dcct,np.concatenate((a_prev,xt),axis=0).T)
